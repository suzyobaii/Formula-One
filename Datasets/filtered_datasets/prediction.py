...
# Update file paths relative to your current script location
...
# Drop drivers who don't have complete data for training
...
y = df_model['2024_Points']  # We're assuming next year's points follow similar trends

# Train the model
model = RandomForestRegressor(n_estimators=100, random_state=42)
model.fit(X, y)

# Prepare data for all drivers (fill NaNs with 0)
X_all = df[['2021_Points', '2022_Points', '2023_Points', '2024_Points']].fillna(0)

# Predict 2025 points
df['2025_Points'] = model.predict(X_all).round(1)

# Calculate standings based on predicted points
